var updateMonitorGraph = function () {
    ...
    d3.json(url, function (error, json) {
        ...
        var bar = chart
            ...
            .attr('transform', function (d) { return 'translate(' + (i++) * 10 + ',0)'; });
        var chart = d3
        ...
        var verticalScale = d3.scale.linear()
            .domain([100, 5000])
            .range([130, 10]);
        var minTime = d3.min(json.monitors.monitor[0].responsetime, function (d) { return format.parse(d.datetime); });
        ...
        var bar = chart
            ...
            .attr('transform', function (d) { return 'translate(' + timeScale(format.parse(d.datetime)) + ',' + verticalScale(d.value) + ')'; });
    });
};
updateMonitorGraph();
